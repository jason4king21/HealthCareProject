...
{% macro load_snf_quality_reporting_program_provider_data_incremental() %}
    ...
    {% for row in file_list_results %}
        ...
        {% if file_name.endswith('.csv') and 'Skilled_Nursing_Facility_Quality_Reporting_Program_Provider_Data' in file_name %}
            ...
            {% if check_result.columns[0].values()[0] == 0 %}
                ...
                {% set copy_sql %}
                    COPY INTO HEALTHCARE.RAW.SNF_QUALITY_REPORTING_PROGRAM_PROVIDER_DATA
                ...
                {% set insert_sql %}
                    ...
                    VALUES ('{{ file_name }}', CURRENT_TIMESTAMP, 'HEALTHCARE.RAW.SNF_QUALITY_REPORTING_PROGRAM_PROVIDER_DATA');
                {% endset %}
