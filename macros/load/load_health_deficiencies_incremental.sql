...
{% macro load_health_deficiencies_incremental() %}
    ...
    {% for row in file_list_results %}
        ...
        {% if file_name.endswith('.csv') and 'NH_HealthCitations_' in file_name %}
            ...
            {% if check_result.columns[0].values()[0] == 0 %}
                ...
                {% set copy_sql %}
                    COPY INTO HEALTHCARE.RAW.HEALTH_DEFICIENCIES
                ...
                {% set insert_sql %}
                    ...
                    VALUES ('{{ file_name }}', CURRENT_TIMESTAMP, 'HEALTHCARE.RAW.HEALTH_DEFICIENCIES');
                {% endset %}
