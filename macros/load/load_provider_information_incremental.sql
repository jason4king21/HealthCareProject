...
{% macro load_provider_information_incremental() %}
    ...
    {% for row in file_list_results %}
        ...
        {% if file_name.endswith('.csv') and 'NH_ProviderInfo_' in file_name %}
            ...
            {% if check_result.columns[0].values()[0] == 0 %}
                ...
                {% set copy_sql %}
                    COPY INTO HEALTHCARE.RAW.PROVIDER_INFORMATION
                ...
                {% set insert_sql %}
                    ...
                    VALUES ('{{ file_name }}', CURRENT_TIMESTAMP, 'HEALTHCARE.RAW.PROVIDER_INFORMATION');
                {% endset %}
