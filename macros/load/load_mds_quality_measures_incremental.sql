...
{% macro load_mds_quality_measures_incremental() %}
    ...
    {% for row in file_list_results %}
        ...
        {% if file_name.endswith('.csv') and 'NH_QualityMsr_MDS_' in file_name %}
            ...
            {% if check_result.columns[0].values()[0] == 0 %}
                ...
                {% set copy_sql %}
                    COPY INTO HEALTHCARE.RAW.MDS_QUALITY_MEASURES
                ...
                {% set insert_sql %}
                    ...
                    VALUES ('{{ file_name }}', CURRENT_TIMESTAMP, 'HEALTHCARE.RAW.MDS_QUALITY_MEASURES');
                {% endset %}
